...
void ControlSystem::reset_timers(){
    ...
    for(auto i : devices){
        i->reset_timer(house_time);
        if(dynamic_cast<ProgrammedCycle*>(i) != NULL && i->get_status()) add_action_to_vector( i->get_name(),house_time + dynamic_cast<ProgrammedCycle *>(i)->get_work_period(), false, true);
    }
}
...
std::string ControlSystem::remove_timer(const std::string& name){
    ...
    if(pos != -1){// se lo trovo allora in pos ho la posizione nell'array del dispositivo richiesto
        ...
        if(dynamic_cast<ProgrammedCycle*>(devices[pos]) != NULL) add_action_to_vector(devices[pos]->get_name(), dynamic_cast<ProgrammedCycle*>(devices[pos])->get_work_period() + house_time, false, true);
        return output;
    }
...
std::string ControlSystem::set_device_on(const std::string& name){
    ...
    if(pos == -1)throw std::invalid_argument("Il nome inserito non è presente tra i dispositivi disponibili");
        ...
        if(!devices[pos]->get_status() && dynamic_cast<ProgrammedCycle*>(devices[pos]) != NULL) add_action_to_vector(devices[pos]->get_name(),house_time + dynamic_cast<ProgrammedCycle *>(devices[pos])->get_work_period(), false, true);
...
std::string ControlSystem::set_timer_device(const std::string& name , const std::string& start, const std::string& end){
    ...
    if(end_time == 2000 && dynamic_cast<ProgrammedCycle*>(devices[pos]) != NULL)end_time = start_time + dynamic_cast<ProgrammedCycle *>(devices[pos])->get_work_period();
    if(end_time == 2000 ){
        output = devices[pos]->setTimer(house_time, start_time);
        add_action_to_vector(devices[pos]->get_name(), start_time, true);
    }
    else{
        if(dynamic_cast<ProgrammedCycle*>(devices[pos]) != NULL){
